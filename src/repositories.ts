...
export function createRepositories(database: DatabaseSync): Repositories {
  ...
  return {
    ...
    workspaceLeases: { get: get("workspace_leases", lease), insert: (r) => { database.prepare(`INSERT INTO workspace_leases
      (id, workspace_id, mode, owner_session_id, owner_batch_id, acquired_at, expires_at, released_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(r.id, r.workspaceId, r.mode, r.ownerSessionId, r.ownerBatchId, r.acquiredAt, r.expiresAt, r.releasedAt); }, activeWriter: (id) => { const row = database.prepare("SELECT * FROM workspace_leases WHERE workspace_id = ? AND mode = 'writer' AND released_at IS NULL").get(id); return row ? lease(row) : undefined; } },
