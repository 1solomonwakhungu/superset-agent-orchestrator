...
- `npx tsx --test --test-skip-pattern="real Superset CLI responses" ...`: 101 tests passed
- `npm pack --dry-run`: passed
- `git diff --check`: passed

The unfiltered `npm run verify` built successfully but had 99/101 tests pass: one
pre-existing transient test cleanup race returned `ENOTEMPTY`, and the real
Superset CLI smoke test failed because the prohibited executable is unavailable.
The complete suite passed 101/101 when that external smoke test was excluded.
