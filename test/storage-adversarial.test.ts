...
test("retention reclaims expired readers without revoking an unreleased writer", async () => {
  await fixture((directory) => {
    ...
    const insertLease = storage.database.prepare(`INSERT INTO workspace_leases
      (id, workspace_id, mode, owner_session_id, owner_batch_id, acquired_at, expires_at, released_at, state)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
    insertLease
      .run("writer", "workspace", "writer", null, "batch", "2026-01-01T00:00:00.000Z",
        "2026-01-02T00:00:00.000Z", null, "active");
    insertLease.run("expired-reader", "workspace", "read-only", null, "batch", "2026-01-01T00:00:00.000Z",
      "2026-01-02T00:00:00.000Z", "2026-01-02T00:00:00.000Z", "released");
    insertLease
      .run("active-reader", "workspace", "read-only", null, "batch", "2026-07-23T00:00:00.000Z",
        "2026-07-25T00:00:00.000Z", null, "active");
