...
The orchestrator owns a standalone SQLite registry and a temporary JSON lifecycle projection while repository migration is in progress. Neither opens, imports, copies, or derives a path from Superset Desktop's databases, manifests, logs, or temporary files. Both use orchestrator-specific owner-only directories, no-follow validation, single-link regular files, and atomic replacement. SQLite remains the target repository for durable entities; the JSON projection must not be described as the SQLite registry or bypass its merged filesystem controls.
