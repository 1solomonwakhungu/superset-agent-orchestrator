...
| C-CMD-01 fixed executable provenance | `assertPinnedExecutable` accepts only an absolute path or a bare command name and is applied inside `runProcess` before `spawn` | T-CMD-02 |
| C-CMD-02 allowlisted argv templates | Discovery uses fixed argument arrays with `shell: false`; `assertFixedArguments` rejects NUL, control, and newline bytes; `assertDataOperand` rejects option-leading and separator-bearing operands such as a resolved workspace path | T-CMD-01, T-CMD-02, T-CMD-03 |
